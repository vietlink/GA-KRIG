...
public class Powvargram implements Func_Doub_To_Doub {
    ...
    public Powvargram(final BitSet[][] x, final double[] y) {
        this(x, y, 1.5, 0.0);
    }

    public Powvargram(final BitSet[][] x, final double[] y, final double beta) {
        this(x, y, beta, 0.0);
    }

    public Powvargram(final BitSet[][] x, final double[] y, final double beta, final double nug) {
        ...
        for (i = 0; i < npt; i++)
            for (j = i + 1; j < npt; j++) {
                rb = 0.0;
                for (k = 0; k < ndim; k++)
                    rb += SQR(x[i][k].cardinality() - x[j][k].cardinality());
                rb = pow(rb, 0.5 * beta);
